...
So whenever `fonts/<language>/font.ini` exists and parses, **TrueType takes over
...
removing `fonts/<language>/font.ini` is the switch.
...
Fonts are configured per language directory, mirroring how the engine already
locates localized text:
...
| `fileName` | Font file, relative to the same `fonts/<language>/` directory. |
